...
def plot_activity(query, output, data_grain=60, tick_grain=120):
    ...
    timestamps = [datetime.strptime(row.timestamp.split(
        '.')[0], '%Y-%m-%d %H:%M:%S') for row in rows]
    ...
    ticks = [start_time + i * timedelta(minutes=data_grain) for i in range(
        int((end_time - start_time).total_seconds() / (data_grain * 60)) + 1)]

    all_keyboard_counts = np.zeros(len(ticks))
    all_click_counts = np.zeros(len(ticks))

    for i, tick in enumerate(ticks):
        relevant_indices = [j for j, t in enumerate(
            timestamps) if tick <= t < tick + timedelta(minutes=data_grain)]
        all_keyboard_counts[i] = sum(keyboard_counts[j]
    ...
    if (end_time - start_time) > timedelta(days=1):
        formatted_ticks = [timestamp.strftime(
            '%Y-%m-%d %H:%M') for timestamp in ticks]
    else:
    ...
    plt.bar(index, all_click_counts,
            bottom=all_keyboard_counts, label='Click Count')
...
def recent_activity_categories():
    ...
    for rounded_timestamp, dominant in dominant_categories.items():
        ...
        other_count = sum(cat['cnt'] for cat in windows[rounded_timestamp]
                          if cat['category'] != dominant['category'])
    ...
    other_counts = [
        sum(cat['cnt'] for cat in windows[t] if cat['category']
            != dominant_categories[t]['category'])
        for t in timestamps
    ]

    unique_categories = set(
        dominant_categories[t]['category'] for t in timestamps)
    ...
    remaining_categories = [
        cat for cat in unique_categories if cat not in predefined_colors]
    ...
    category_colors = {**predefined_colors, **
                       dict(zip(remaining_categories, remaining_colors))}

    # 6. Continue with your plotting logic
    # e.g., using dominant_colors in your plt.bar() function
    dominant_colors = [
        category_colors[dominant_categories[t]['category']] for t in timestamps]

    # Plotting
    plt.figure(figsize=(10, 6))

    plt.bar(timestamps, dominant_counts,
            color=dominant_colors, label='Dominant Category')
    plt.bar(timestamps, other_counts, color='black',
            bottom=dominant_counts, label='Other Categories')
    ...
    if len(timestamps) > 0:
        plt.xticks(
            [timestamps[0], timestamps[len(timestamps)//2], timestamps[-1]])
...
query_results = pd.DataFrame(
    rows, columns=["window_title", "timestamps", "keyboard_count", "click_count"])
...
# Assume p75_mouse_events is the 8th column in your SQL output
avg_mouse_events = [x[3] for x in data]
...
plt.figure(figsize=(12, 6))
bar1 = plt.bar(index, avg_keyboard_events, bar_width,
               label='avg_keyboard_events')
bar2 = plt.bar(index, avg_mouse_events, bar_width,
               bottom=avg_keyboard_events, label='avg_mouse_events')
...
sorted_indices_with_data = [
    index for index in sorted_indices if keyboard_events[index] > 0]

top_5_indices = sorted_indices_with_data[-5:]
# Ensure that we only consider non-zero data for the bottom 5
bottom_5_indices = sorted_indices_with_data[:5] if len(
    sorted_indices_with_data) > 5 else []

# Create a list for the colors of the bars
colors = [
    'blue' if i not in top_5_indices and i not in bottom_5_indices else 'green' if i in top_5_indices else 'red' for i in range(24)]
...
plt.bar(indices, keyboard_events, bar_width,
        label='Keyboard Events', color=colors)
...
plt.bar(indices, keyboard_events, bar_width,
        label='Keyboard Events', color='blue')

# Plotting the mouse events on top of the keyboard events
plt.bar(indices, mouse_events, bar_width, label='Mouse Events',
        color='orange', bottom=keyboard_events)
...
rect = Rectangle(bottom_left_corner, width, height,
...
# Populate the activity_data array
for row in rows:
    weekday, hour, ratio = int(row[0]), int(
        row[1]), float(row[2])  # Ensure ratio is a float
